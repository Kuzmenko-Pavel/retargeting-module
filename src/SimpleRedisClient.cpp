...
bool SimpleRedisClient::zadd(const std::string &key, int score, const std::string & member)
{
    redis_send(RC_INT, "ZADD %s %d %s\r\n", key.c_str(), score, member.c_str());
    return testRetun();
}

bool SimpleRedisClient::zadd(const std::string &key, int score, long member)
{
    redis_send(RC_INT, "ZADD %s %d %ld\r\n", key.c_str(), score, member);
    return testRetun();
}

bool SimpleRedisClient::set(const std::string &key, const std::string & member)
{
    redis_send( RC_INLINE, "SET %s %s\r\n", key.c_str(), member.c_str());
    return testRetun();
}

bool SimpleRedisClient::testRetun() const
{
    if(data && data[0] == '1')
    {
        return true;
...
bool SimpleRedisClient::setex(const std::string &key, const std::string & member, int seconds)
{
    redis_send(RC_INLINE, "SETEX %s %d %s\r\n",key.c_str(),seconds, member.c_str());
    return testRetun();
}
